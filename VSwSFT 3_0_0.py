"""
    ...
    Building on the code from VS 2.7.1
    Using 6 Experimental Conditions:
        3 Response Styles:
            Yes/No (same as previous experiments)
            Mouse (always visible; move and click on target)
            Eyes (fixate the target)
        2 Stimuli Motions:
            Static (same as previous)
            Dynamic (waterfall)
            
    Slight change from previous code: using 'm' and 'e' instead of 'd' for response style codes
"""
...
expName='VSwSFT_DSDR'    # Name of the Experiment
expInfo={'participant':'TEST', 'session':'1', 'condition (s/d)':('s','d'), 'response style (s/d)':('s','m','e')}  # condition: s = static stimuli, d = dynamic stimuli; response style: s = click when found, m = click on target, e = fixate on target
...
if ((expInfo['condition'] != 's' and expInfo['condition'] != 'd') or (expInfo['response style'] != 's' and expInfo['response style'] != 'm' and expInfo['response style'] != 'e')): core.quit() # cannot run experiment without a specified condition and response style
...
mhead = 'data' + os.path.sep + 'mousetracking'
ehead = 'data' + os.path.sep + 'eyetracking'
...
if not os.path.isdir(mhead):
    os.makedirs(mhead) #if this fails (e.g. permissions) we will get error
if not os.path.isdir(ehead):
    os.makedirs(ehead) #if this fails (e.g. permissions) we will get error
...
# Don't need to make an eyetracking file because the tracker should do this. Just move that file to the appropriate file at the end of the session.
file = filename+'.txt'
sfile = sfilename+'_stimuli.txt'
logFile=open(file, 'a')
stimFile=open(sfile, 'a')

# Don't need to track mouse in the eye-reponse condition
if expInfo['response style'] != 'e':
    mfilename = mhead + os.path.sep + '%s_%s' %(expInfo['participant'], expInfo['date'])
    mfile = mfilename+'_mousetracking.txt'
    mtFile=open(mfile, 'a')
    mtFile.write("Date\tSubject\tSession\tCondition\tDistractors\tTrial\tTime\tMouseX\tMouseY\n")
...
dInstructions = visual.TextStim(win=win, ori=0, name='DRinstructions',
    ...
    font='Arial', pos=[0, 0], units="pix",height=40,wrapWidth=DISPLAY_DIMENSIONS[0]-100,
    color='black', colorSpace='rgb', opacity=1, depth=0.0)
readyCue = (visual.Circle(win=win, radius=STIM_RAD, lineColor='white', fillColor='white', pos=(0,0), units='pix', lineColorSpace='rgb255',fillColorSpace='rgb255', lineWidth=2),
    visual.Line(win=win, start=(-STIM_RAD/1,0), end=(STIM_RAD/1,0), lineColor='black', lineWidth=2), visual.Line(win=win, start=(0,-STIM_RAD/1), end=(0,STIM_RAD/1), lineColor='black', lineWidth=2))
...
def VisualSearch(nBlocks, conditionsFile):
    ...
    for thisTrial in block:
        ...
        # Record mouse positions for response style=='m' condition
        #mouseLog = [[0],[int(.5*DISPLAY_DIMENSIONS[0])], [int(.5*DISPLAY_DIMENSIONS[1])]] # First entry should be at time 0 and in the center of the screen
        mouseLog = [[],[],[]] # time,X,Y
        
#------- READY SCREEN --------
        toggle(readyCue, True)
        #readyCue.setAutoDraw(True)
        win.flip()
        # When implemented... Replace sleep with gc
        sleep(READY_TIME)
        toggle(readyCue, False)
        ...
        while continueRoutine and routineTimer.getTime()>0:
            ...
            if trialComponents['mouse'].status==STARTED and sum(response)==0: #only update if started and no previous response has been recorded
                if expInfo['response style']=='m': #Log mouse position
                    mouseLog[0].append(t); mouseLog[1].append(trialComponents['mouse'].getPos()[0]); mouseLog[2].append(trialComponents['mouse'].getPos()[1])
                    ...
                    if buttons[0] == 1:
                        if expInfo['response style'] == 'm':
                            ...
                            if on_stim:
                                response = [1,0]
                                rt = times[0]
                                #clickLoc = trialComponents['mouse'].getPos() # should be set above
                                trialComponents['mouse'].setVisible(False)
                                continueRoutine = False
        ...
        if expInfo['response style'] == 's' and response[0] == 1:
            ...
            while get_response:
                ...
                if event.getKeys(["escape"]):
                    core.quit()
                mouseLog[0].append(trialClock.getTime()); mouseLog[1].append(trialComponents['mouse'].getPos()[0]); mouseLog[2].append(trialComponents['mouse'].getPos()[1])
                #check for response
        ...
        for i in range(len(mouseLog[0])):
            mtStr = str(expInfo['date']) + "\t" + str(expInfo['participant']) + "\t" + str(expInfo['session']) + "\t" + str(thisTrial.type) + "\t" + str(thisTrial.distractors) + "\t" + str(trialcounter) + "\t" + str(mouseLog[0][i]) + "\t" + str(mouseLog[1][i]) + "\t" + str(mouseLog[2][i]) + "\n"
            mtFile.write(mtStr)
